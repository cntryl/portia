...
public sealed class ScopeHandler(IRequestBus bus, IConsumerScope scope, IConsumerEffects effects)
    : IRequestHandler<ScopeRequest>
{
    public async ValueTask<Result> HandleAsync(IRequestContext<ScopeRequest> context, CancellationToken ct)
    {
        ...
        return context.Request.Behavior == 3
            ? Result.Failure(new RequestError(RequestErrorKind.Validation, "Terminal rejection"))
            : Result.Success;
    }
}
