...
public sealed class ContextSaveTests
{
    ...
    public async Task FailedSaveFreezesAttributionAndRetryPreservesSerializedEvents(bool audit)
    {
        ...
        var actor = new ClaimsPrincipal(new ClaimsIdentity([new Claim(ClaimTypes.NameIdentifier, "alice", ClaimValueTypes.String, "accounts")], "test"));
        ...
        _ = await Assert.ThrowsAsync<InvalidOperationException>(async () => await repository.SaveAsync(account, replacement));
    ...
    public async Task CancelledSaveDoesNotFreezeAttribution()
    {
        ...
        _ = await Assert.ThrowsAnyAsync<OperationCanceledException>(async () => await repository.SaveAsync(account, first, cancellation.Token));
    ...
    public async Task ConflictingMetadataRejectsWholeBatchBeforeStampingOrAppending()
    {
        ...
        _ = await Assert.ThrowsAsync<InvalidOperationException>(async () => await repository.SaveAsync(account, context));
    ...
    sealed class ConflictingFactory : IDomainEventMetadataFactory
    {
        public DomainEventMetadata Create(Uuid aggregateId, ulong aggregateVersion)
            => new(Uuid.CreateVersion4(), aggregateId, aggregateVersion, DateTimeOffset.UtcNow,
                CorrelationId: aggregateVersion == 2 ? Uuid.CreateVersion4() : null);
    }
    ...
    sealed class FailingStore : IEventStore
    {
        ...
        public ValueTask AppendAsync(EventStreamAddress stream, ulong expectedStreamPosition, IReadOnlyList<DomainEvent> events, CancellationToken ct = default)
        {
            ...
            return Writes.Count == 1 ? throw new IOException("Unconfirmed append") : _inner.AppendAsync(stream, expectedStreamPosition, events, ct);
        }
