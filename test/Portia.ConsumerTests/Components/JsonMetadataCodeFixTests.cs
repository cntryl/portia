...
public sealed class JsonMetadataCodeFixTests
{
    ...
    /// <summary>
    ///     Verifies that when the part carrying <c>[PortiaJsonContext]</c> is generated code, which its tool
    ///     would overwrite, the root goes on the application's own part instead of creating a second context.
    /// </summary>
    [Fact]
    public async Task AddsTheRootToTheHandWrittenPartWhenTheMarkerPartIsGenerated()
    {
        using var workspace = new AdhocWorkspace();
        ...
        Assert.Contains("typeof(global::App.Query)", await DocumentTextAsync(changed, "AppJsonContext.cs"),
            StringComparison.Ordinal);
        Assert.DoesNotContain("typeof", await DocumentTextAsync(changed, "AppJsonContext.g.cs"),
            StringComparison.Ordinal);
    }
