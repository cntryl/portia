...
public sealed class FleetBrokerTests
{
    ...
    public async Task HealthyScaleOutThenDepartureOrCrashRestoresAllPartitions(bool crash)
    {
        ...
        var activeA = new ConcurrentDictionary<string, ulong>();
        var activeB = new ConcurrentDictionary<string, ulong>();
        ...
        var runA = runnerA.RunAsync(partitions, (route, authority, ct) => Hold(route, authority, activeA, startsA, ct), options, cancelA.Token);
        Task? runB = null;
        try
        {
            await Until(() => activeA.Count == partitions.Length);
            var original = activeA.ToDictionary();
            runB = runnerB.RunAsync([.. partitions.Reverse()], (route, authority, ct) => Hold(route, authority, activeB, null, ct), options with { WorkerId = workerB }, cancelB.Token);
            ...
            Assert.All(activeB, entry => Assert.True(entry.Value > original[entry.Key]));
            if (crash)
    ...
    public async Task UnrenewedMembershipAndPartitionExpireBeforeSurvivorTakesOver()
    {
        ...
        var acquired = new TaskCompletionSource<LeaseAuthority>(TaskCreationOptions.RunContinuationsAsynchronously);
        ...
        var run = runner.RunAsync([partition], (route, authority, ct) =>
        {
            _ = acquired.TrySetResult(authority);
        ...
        try
        {
            ...
            var authority = await acquired.Task.WaitAsync(TimeSpan.FromSeconds(15));
            Assert.True(authority.FencingToken > 0);
        }
        finally { cancellation.Cancel(); await run; }
    }

    static async Task Hold(string route, LeaseAuthority authority, ConcurrentDictionary<string, ulong> active,
        ConcurrentDictionary<string, int>? starts, CancellationToken ct)
    {
        Assert.True(active.TryAdd(route, authority.FencingToken));
