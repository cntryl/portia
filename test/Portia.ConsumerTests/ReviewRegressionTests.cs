...
public sealed class ReviewRegressionTests
{
    ...
    public async Task PropertyConverterUsesApplicationJsonContract()
    {
        ...
        var (status, body) = await HttpConsumerScenario.RunAsync(assembly, "/binding", /*lang=json,strict*/ "{\"value\":\"0x10\"}");
        Assert.Equal(200, status);
    ...
    {
        var assembly = HttpConsumerScenario.Compile(
            "public sealed record Binding([property: JsonNumberHandling(JsonNumberHandling.Strict)] int Value) : IRequest<string>, ICallable;",
            "request.Value.ToString(CultureInfo.InvariantCulture)",
            "app.MapPortiaPost<Binding, string>(\"/binding\");");
