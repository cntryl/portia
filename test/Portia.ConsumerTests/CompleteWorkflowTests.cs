    ...
    public async Task SameBusinessHandlerPersistsAcrossDirectHttpRpcAndQueueWithIndependentAudits(bool fitzStore)
    {
        ...
        if (fitzStore)
        {
            _ = builder.Services.AddSingleton<IEventStore>(provider =>
                new FitzEventStore(client.Stream, provider.GetRequiredService<IDomainEventSerializer>()));
        }
        ...
        _ = builder.Services.AddSingleton<IRequestQueuePublisher>(
            new FitzRequestQueuePublisher(client.Queue, requestSerializer));
        ...
        _ = builder.Services.AddSingleton<IRequestQueueConsumer>(new FitzRequestQueueConsumer(client.Queue,
            requestSerializer, "queue://consumer/business/" + id, ConsumerJson.Catalog()));
        ...
        try
        {
            ...
            {
                Assert.True((await scope.ServiceProvider.GetRequiredService<IRequestBus>()
                    .SendAsync(new DepositAccount(id, 5), RequestActor.System)).IsSuccess);
            }
            ...
            await using (var scope = app.Services.CreateAsyncScope())
            {
                ...
                Assert.All(records.Where(record => !record.Event.Metadata.IsAudit),
                    record => Assert.Equal(account.Stream, record.Stream));
            }

            foreach (var component in new[]
                         { "first-projector", "second-projector", "first-reactor", "second-reactor" })
            {
                await effects.WaitForAsync(component, 5, id);
                var applied = effects.Items.Where(item => item.Component == component && item.AggregateId == id)
                    .ToArray();
