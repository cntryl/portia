...
/// <summary>
///     Describes how one registered projector resolves and runs a pass. Built by
///     <c>PortiaBuilder.AddProjector</c>; applications do not construct it.
/// </summary>
public sealed class ProjectorRegistration : IWorkloadDescriptor
{
    ...
    ProjectorRegistration(
        ...
        Func<ProjectorRunner, IServiceProvider, ProjectionCheckpoint, ProjectionRunOptions?, CancellationToken,
    ...
    public static ProjectorRegistration Create<TProjector>()
        ...
        static async (services, options, ct) =>
        {
            ...
            var checkpoint = await projector.Store
                .LoadCheckpointAsync(new CheckpointIdentity(projector.Name, projector.Pattern, options?.RebuildId), ct)
