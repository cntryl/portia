...
static class EventStreamOffsets
{
    public static ulong GetNextOffset(EventStreamPattern pattern, DomainEventRecord record) => pattern.Scope switch
    {
        ...
        EventStreamPatternScope.Area => checked((record.AreaOffset ?? throw new InvalidOperationException("Missing area checkpoint offset.")) + 1),
        EventStreamPatternScope.Realm => checked((record.RealmOffset ?? throw new InvalidOperationException("Missing realm checkpoint offset.")) + 1),
