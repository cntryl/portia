...
public interface IRequestPipelineBehavior<in TRequest> where TRequest : IRequest
{
    ...
    ValueTask<Result> HandleAsync(IRequestContext<TRequest> context, RequestPipelineNext continuation, CancellationToken ct);
}
...
{
    /// <summary>Executes this behavior and, when appropriate, the remainder of the pipeline.</summary>
    ValueTask<Result<TOut>> HandleAsync(IRequestContext<TRequest> context, RequestPipelineNext<TOut> continuation, CancellationToken ct);
}
...
public interface IStreamRequestPipelineBehavior<in TRequest, TOut> where TRequest : IStreamRequest<TOut>
{
    ...
    IAsyncEnumerable<TOut> HandleAsync(IRequestContext<TRequest> context, StreamRequestPipelineNext<TOut> continuation, CancellationToken ct);
}
