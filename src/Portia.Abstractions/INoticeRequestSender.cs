...
public interface INoticeRequestSender
{
    ...
    ///     The publishing actor's raw bearer token, or <see langword="null" />
    ///     for an unauthenticated actor — re-validated on the receiving side, not just forwarded as-is.
    /// </param>
    /// <param name="ct">A token that can cancel the operation.</param>
    /// <returns>A task representing the publish.</returns>
    ValueTask PublishAsync<TRequest>(TRequest request, RequestRouteValues routeValues, string? actorToken,
