...
public sealed record FleetRunOptions
{
    ...
    internal void Validate(IReadOnlyCollection<string> partitions)
    {
        ...
        {
            var parts = partition?.StartsWith("lease://", StringComparison.Ordinal) == true
                ? partition[8..].Split('/')
                : [];
            if (parts.Length != 3 || !parts.All(IsSegment))
            {
                throw new ArgumentException(
                    "Partition must be lease://{realm}/{area}/{resource} with exact nonblank segments.",
                    nameof(partitions));
            }

            if (partition!.StartsWith(prefix, StringComparison.Ordinal))
            {
                throw new ArgumentException("Membership must use a dedicated area outside partition routes.",
                    nameof(partitions));
            }
