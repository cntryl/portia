...
/// <c>Microsoft.Extensions.DependencyInjection</c> and Portia.Fitz (for
/// <c>JsonDomainEventSerializer</c>), also emits
/// <c>IServiceCollection.AddPortiaGeneratedDomainEventSerialization()</c>, resolving a fully
/// wired <c>IDomainEventSerializer</c> from DI in one call.
/// </summary>
...
public sealed class DomainEventCatalogGenerator : IIncrementalGenerator
{
        ...
        Compilation compilation)
    {
        ...
        if (compilation.GetTypeByMetadataName(JsonDomainEventSerializerMetadataName) is null)
            return;

        var diBuilder = new StringBuilder();
        _ = diBuilder.AppendLine("// <auto-generated />");
        _ = diBuilder.AppendLine("using Microsoft.Extensions.DependencyInjection;");
        _ = diBuilder.AppendLine();
        _ = diBuilder.AppendLine("namespace Cntryl.Portia;");
        _ = diBuilder.AppendLine();
        _ = diBuilder.AppendLine("static class DomainEventSerializationGeneratedServiceCollectionExtensions");
        _ = diBuilder.AppendLine("{");
        _ = diBuilder.AppendLine("    public static global::Microsoft.Extensions.DependencyInjection.IServiceCollection AddPortiaGeneratedDomainEventSerialization(this global::Microsoft.Extensions.DependencyInjection.IServiceCollection services)");
        _ = diBuilder.AppendLine("    {");
        _ = diBuilder.AppendLine("        global::System.ArgumentNullException.ThrowIfNull(services);");
        ...
        context.AddSource("DomainEventSerializationGeneratedServiceCollectionExtensions.g.cs", diBuilder.ToString());
    }
