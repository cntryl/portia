...
public sealed class JsonMetadataCodeFixProvider : CodeFixProvider
{
    ...
    public override async Task RegisterCodeFixesAsync(CodeFixContext context)
    {
        ...
        if (contexts.Length == 0)
        {
            context.RegisterCodeFix(CodeAction.Create($"Create PortiaJsonContext for {typeName}",
                    ct => CreateContextAsync(context.Document.Project, typeName, ct), "Portia.CreateJsonContext"),
                diagnostic);
            return;
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var text =
            $"System.Text.Json.Serialization.JsonSerializable(typeof({typeName.Replace("global::", string.Empty)}))";
        if (declaration.AttributeLists.SelectMany(list => list.Attributes)
            .Any(attribute => attribute.ToString() == text))
        {
            return solution;
        }
            ...
            SyntaxFactory.AttributeArgumentList([
                SyntaxFactory.AttributeArgument(
                    SyntaxFactory.TypeOfExpression(
                        SyntaxFactory.ParseTypeName(typeName.Replace("global::", string.Empty))))
            ]));
    ...
    static async Task<Solution> CreateContextAsync(Project project, string typeName,
        CancellationToken cancellationToken)
    {
