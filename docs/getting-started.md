...
replicas, and `UseSingleProcessWorkloads()` owns them all in a single worker replica; startup
fails unless exactly one coordinator is registered. `WorkloadScope.PerTenant` requires an
